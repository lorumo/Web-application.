...
function openForm() {
    ...
    document.getElementById("authorcom").value = localStorage.getItem("userName");
...
function findByTag(targetElm, artIdFromHash, offsetFromHash, totalCountFromHash) {
    ...
    function reqListener() {
        if (this.status == 200) {
            ...
            // Сбор промисов для запросов на получение контента для каждой статьи
            const articlePromises = responseJSON.articles.map(fetchArticleContent);

            // Используем Promise.all для дожидания завершения всех запросов
